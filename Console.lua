...
local function MainCommand( msg )
	...
	elseif command == "matchcrit" then
		if rest == "on" or rest == "true" then
			Me.db.matchCritEnabled = true
		elseif rest == "off" or rest == "false" then
			Me.db.matchCritEnabled = false
		else
			Me.db.matchCritEnabled = not Me.db.matchCritEnabled
		end
		Me.Print( "2d12 matching-dice crit: " .. ( Me.db.matchCritEnabled and "|cff00ff00on|r" or "|cffff0000off|r" ) .. "." )
	...
	elseif command == "atk" or command == "def" then
		...
		Me.PrintSystemMessage( "- /dice (XdY[+/-]Z)  -- roll dice" )
		...
		Me.PrintSystemMessage( "- /eph matchcrit (on || off)  -- colour matching 2d12 rolls green" )
